...
func main() {
	...
	if err = (&controllers.EffectiveScheduleReconciler{
		Client: mgr.GetClient(),
		Log:    ctrl.Log.WithName("controllers").WithName("EffectiveSchedule"),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "EffectiveSchedule")
		os.Exit(1)
	}
